...
export const checkAccessTokenMiddleware = (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.cookies) return sendErrorResponse('Необходима авторизация', 401)
    if (!req.cookies.accessToken) return sendErrorResponse('Сессия не найдена, войдите снова', 401)
...
export const checkRefreshTokenMiddleware = (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.cookies) return sendErrorResponse('Необходима авторизация', 401)
    if (!req.cookies.refreshToken) return sendErrorResponse('Сессия не найдена, войдите снова', 401)
    jwt.verify(
